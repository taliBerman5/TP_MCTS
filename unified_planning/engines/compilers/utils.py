...
def create_effect_with_given_subs(
    ...
    return Effect(new_fluent, new_value)


def create_precondition_with_given_subs(
    problem: Problem,
    old_precondition: Precondition,
    simplifier,
    subs: Dict[Expression, Expression],
) -> Optional[Precondition]:
    new_fluent = old_precondition.fluent.substitute(subs)
    new_value = old_precondition.value.substitute(subs)
    return Precondition(new_fluent, new_value)


def create_probabilistic_effect_with_given_subs(
    problem: Problem,
    old_probabilistic_effect: ProbabilisticEffect,
    simplifier,
    subs: Dict[Expression, Expression],
    c_subs: Dict["up.model.parameter.Parameter", "up.model.fnode.FNode"],
) -> Optional[ProbabilisticEffect]:

    new_fluents = []
    for f in old_probabilistic_effect.fluents:
        new_fluents.append(f.substitute(subs))
    def fun(_state, _):
        assert old_probabilistic_effect is not None
        return old_probabilistic_effect.probability_function(_state, c_subs)

    return ProbabilisticEffect(new_fluents, fun)
...
def create_action_with_given_subs(
    ...
    if isinstance(old_action, InstantaneousAction):
        ...
        for p in old_action.preconditions:
            new_precondition = create_precondition_with_given_subs(problem, p, simplifier, subs)
            if new_precondition is not None:
                # We try to add the new precondition, but a compiler might generate conflicting preconditions,
                # so the action is just considered invalid
                try:
                    new_action._add_precondition_instance(new_precondition)
        ...
        for e in old_action.effects:
            ...
            if new_effect is not None:
                ...
                try:
                    ...
                    return None
        for pe in old_action.probabilistic_effects:
            new_probabilistic_effect = create_probabilistic_effect_with_given_subs(problem, pe, simplifier, subs, c_subs)
            if new_probabilistic_effect is not None:
                # We try to add the new probabilistic effect, but a compiler might generate conflicting effects,
                # so the action is just considered invalid
                try:
                    new_action._add_probabilistic_effect_instance(new_probabilistic_effect)
                except UPConflictingEffectsException:
                    return None
        return new_action
        ...
        new_durative_action._set_duration(old_action.duration)

        for p_type in old_action.preconditions:
            for p in old_action.preconditions[p_type]:
                new_precondition = create_precondition_with_given_subs(problem, p, simplifier, subs)
                if new_precondition is not None:
                    # We try to add the new precondition, but a compiler might generate conflicting preconditions,
                    # so the action is just considered invalid
                    try:
                        new_durative_action._add_precondition_instance(p_type, new_precondition)
                    except UPConflictingEffectsException:
                        return None

        for e in old_action.start_effects:
            new_effect = create_effect_with_given_subs(problem, e, simplifier, subs)
            if new_effect is not None:
                # We try to add the new effect, but a compiler might generate conflicting effects,
                # so the action is just considered invalid
                try:
                    new_durative_action._add_start_effect_instance(new_effect)
                except UPConflictingEffectsException:
                    return None
        ...
        for e in old_action.effects:
            ...
            if new_effect is not None:
                ...
                # so the action is just considered invalid
                try:
                    new_durative_action._add_effect_instance(new_effect)
                except UPConflictingEffectsException:
                    return None
        for pe in old_action.probabilistic_effects:
            new_probabilistic_effect = create_probabilistic_effect_with_given_subs(problem, pe, simplifier, subs,
                                                                                   c_subs)
            if new_probabilistic_effect is not None:
                # We try to add the new probabilistic effect, but a compiler might generate conflicting effects,
                # so the action is just considered invalid
                try:
                    new_durative_action._add_probabilistic_effect_instance(new_probabilistic_effect)
                except UPConflictingEffectsException:
                    return None
